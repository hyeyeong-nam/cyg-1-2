...
#define INTERVAL 25 // sampling interval (unit: ms) 샘플링 주기 25ms로 수정
...
// turn on the LED if the distance is between dist_min and dist_max
 if(dist_raw == 150.0 || dist_raw == 250.0) {
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) {
    reading = last_dist_raw; // return 0 when out of range.
  }
  else if(reading == 0) {
    reading = last_dist_raw;
  }
  last_dist_raw = reading;
  return reading;
